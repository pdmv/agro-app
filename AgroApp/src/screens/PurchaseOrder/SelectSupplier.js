...
const SelectSupplier = () => {
  ...
  const { setSelectedSupplier, selectedSupplier } = useContext(SupplierContext);
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.container}>
          ...
          <ScrollView style={{ marginTop: 20, paddingHorizontal: 16, }}>
            {list && list.map((item) => {
              return (
                <FunctionNav key={item.id} title={item.name} leftIcon="truck" onPress={() => handleDetail({ item })} />
              );
            })}
          </ScrollView>
