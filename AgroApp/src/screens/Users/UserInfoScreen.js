...
import { registerRoleUserThunk, resetError, resetStatus } from '../../redux/userSlice';
...
export default UserInfoScreen = () => {
  ...
  useEffect(() => {
    ...
    } else if (status === 'adding_user_failed') {
      timeoutId = setTimeout(() => {
        setLoading(false);
  ...
  const handleAddUser = () => {
    ...
    dispatch(registerRoleUserThunk({ token, data: userData }));
  };
    ...
    >
      <ScrollView style={styles.container}>
        ...
        <TouchableOpacity style={styles.editButtonContainer} onPress={handleAddUser}>
          <Feather name="plus" size={22} color={Color.white} style={{ marginRight: 10 }} />
      ...
      {/* Info Modal */}
      {showInfoModal && (
        <InfoModal msg="Thêm người dùng thành công!" visible={showInfoModal} onPress={handleInfoOk} />
      )}
    </KeyboardAvoidingView>
  );
};
